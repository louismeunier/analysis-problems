...
1. $a)$ Remember that $abs(sin(x)) <= 1$ for all $x in RR$, thus $ 0 <= abs(sin(n)/n) <= 1/n. $ Since $1/n$ converges to zero, then by the squeeze theorem, so does $sin(n)/n$. \
...
4. We will employ the Cauchy criterion to prove convergence. Let $epsilon > 0$ and fix $m > n >= N$ for some $N$ to be determined later. Then, $ abs(x_m - x_n) & = (1 + 1/(2^2) + dots.c + 1/m^2) - (1 + 1/2^2 + dots.c + 1/n^2) \
...
4. Assume the first direction. Let $epsilon > 0$. By definition, there exists $M > 0$ such that $x > M$ implies $abs(f(x) - L) < epsilon$. Taking $delta := 1/M$, then, this implies that for all $y$ such that $0 < 1/y < 1/delta$, (viewing $x$ as $1/y$) we have $abs(f(1/y) - L) < epsilon$, which $lim_(x -> 0^+) f(1/x) = L$. The inverse implication is very similar.

5. Let $epsilon > 0$ and let $delta := epsilon/K$. Then, $abs(x - y) < delta$ implies $abs(f(x) - f(y)) <= K abs(x - y) < K/K epsilon = epsilon$, so $f$ continuous (uniformly). The classic example of a non-Lipschitz but continuous function is $f(x) := x^2$ on $RR$; to see this, it suffices to take $y = 0$. Then, we see that $ abs(f(x) - f(y)) = x^2, $ so any Lipschitz constant would have to be proportional to $x$, which contradicts the uniformity definition.
